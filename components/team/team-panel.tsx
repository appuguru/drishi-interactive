...
import { X, Plus, Trash2, Crown, User, Users } from 'lucide-react'
...
export function TeamPanel({ project, members, onClose, onMembersChange }: Props) {
  ...
  const updateRole = async (clerkId: string, newRole: MemberRole) => {
    ...
    onMembersChange(members.map(m => m.clerk_id === clerkId ? { ...m, role: newRole } : m))
  }
      ...
      >
        <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-800">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{members.length} members</p>
          </div>
        ...
        <div className="p-6 border-b border-gray-100 dark:border-gray-800">
          <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Add Team Member</p>
        ...
        <div className="p-6 max-h-72 overflow-y-auto space-y-3">
          {members.length === 0 ? (
            ...
            <div key={m.id} className="flex items-center gap-3">
              ...
              <button onClick={() => removeMember(m.id, m.clerk_id)} className="p-1.5 text-gray-400 hover:text-red-500 transition-colors">
